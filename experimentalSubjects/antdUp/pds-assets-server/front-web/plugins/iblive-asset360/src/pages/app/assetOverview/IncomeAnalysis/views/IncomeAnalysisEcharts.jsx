...
import { Button, Empty, Radio } from 'antd-v5';
...
const IncomeAnalysisEcharts = ({
  ...
  const dataRange = useMemo(() => {
    ...
    return {
      startDate: calendarValue.clone().startOf(selectType.get()),
      endDate: calendarValue.clone().endOf(selectType.get()),
    };
  }, [calendarValue]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const option = {
      tooltip: {
        formatter: (params) =>
          `${params.data.name}  ${moneyFormat({ num: params.data.value })}`,
      },
      series: [
        {
          breadcrumb: {
  ...
  useEffect(() => {
    setSelectedStock(tableDataSource[0]);
  }, [tableDataSource]);
  useEffect(() => {
    if (chartRef.current) {
  ...
  const resetTreemap = () => {
    if (chartRef.current) {
      chartRef.current.dispatchAction({
        ...
        targetNode: 'product1-stock', // 这里使用您的根节点ID
      });
    }
  };
  return (
    <div className={styles.incomeAnalysisEcharts}>
      ...
      >
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <div>
            {dataRange?.startDate?.format('YYYY-MM-DD')} ~{' '}
            {dataRange?.endDate?.format('YYYY-MM-DD')}
          </div>
        </div>
