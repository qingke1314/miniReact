...
export default function StockPositionTable({ productCode }) {
  ...
  const gpColumns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <span>
            <span>{moneyFormat({ num: text })}</span>(
            <span>
              {moneyFormat({
                num: record.riseRatio * 100,
                unit: '%',
                sign: record.riseRatio > 0 ? '+' : '',
                needColor: true,
              })}
            </span>
            )
  ...
  const { data: info, run: getInfo, cancel: cancelGetInfo } = useRequest(
    async () => {
      ...
      const res2 = await executeApi({
        serviceId: 'DD_API_FUND_STOCK_NET_RATIO_STAT',
        data: {
          fundCode: productCode,
          businDate: moment().format('YYYYMMDD'),
        },
  ...
  return (
    <div className="blank-card-asset">
      <Row justify="space-between" align="middle" className="m-b-8">
        <div className="important-title">股票TOP10</div>
      </Row>
      <Row>
        <Col span={16}>
          <OverviewTable
            showTotal={false}
            pagination={false}
            dataSource={info?.stockTop10List}
            columns={gpColumns}
            loading={loading}
          />
        </Col>
