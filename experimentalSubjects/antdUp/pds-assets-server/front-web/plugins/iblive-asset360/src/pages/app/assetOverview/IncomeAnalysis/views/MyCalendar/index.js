...
import { calendarType, selectType } from '../../const';
          ...
          <Col span={8} key={year}>
            ...
            >
              <div
                ...
                tabIndex={0}
              >
                <div style={{ color: '#333' }}>{year}</div>
                <div>
                  {calendarData[year]?.income
                    ? moneyFormat({
                        num: calendarData[year]?.income / 10000,
                        needColor: true,
                      })
                    : '--'}
                </div>
...
const App = ({
  ...
  const monthCellRender = (value) => {
    ...
    return (
      <div className={styles.events} style={{ background: colorClass }}>
        <div className={styles.cellHeader}>{value.month() + 1}</div>
        <div>
          {num
            ? moneyFormat({
                num: num / 10000,
      ...
      >
        <Row gutter={8} justify="space-between">
          <Col>
            ...
            <span style={{ marginLeft: 8 }}>(单位：万)</span>
          </Col>
  ...
  return (
    ...
    >
      <div className="important-title">盈亏日历</div>
      <Calendar
        value={calendarValue}
        onSelect={onSelect}
