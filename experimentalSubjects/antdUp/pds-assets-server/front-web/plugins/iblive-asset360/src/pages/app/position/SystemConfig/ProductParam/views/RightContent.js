 ...
 * @LastEditTime: 2024-11-12 14:21:05
 * @Description:
 */
...
export default ({ selectIndex }) => {
  ...
  const updateList = async () => {
    setLoading(true);
    const res = await queryFundCfgAll({
      type,
      name: searchValue.current,
      fundCode: selectIndex?.fundCode,
      astUnitId: selectIndex?.astUnitId,
    });
    const keys = [];
  ...
  const saveList = () => {
    ...
    tableRef.current.validateFields().then(async () => {
      setSaving(true);
      const res = await updateFundCfgUpdate({
