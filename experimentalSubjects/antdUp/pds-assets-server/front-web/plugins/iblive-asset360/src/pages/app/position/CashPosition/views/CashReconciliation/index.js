 ...
 * @LastEditTime: 2024-12-13 11:54:22
...
export default forwardRef(
  ({ headerRender, paramsForList, date, setUpdateDate, updateTree }, ref) => {
    ...
    const columns = [
        ...
        render: (text, record) => (
          ...
          >
            ...
            {record?.warnType === 'POSITION_NOT_ENOUGH' ? (
              <span className={styles.tree_title_status}>头寸不足</span>
            ) : record?.warnType === 'POSITION_LESS' ? (
              <span className={styles.tree_title_status_2}>金额较少</span>
            ) : null}
          </span>
        ),
      },
      ...
      },
      {
        ...
        render: (text, record) =>
          record?.indexLevel !== '2' && (
            <>
              {record?.execStatus === 'success' ? null : (
                // <a
                //   onClick={() => {
    ...
    return (
      <div className={styles.cash_forecast_main}>
        {headerRender('cashReconciliation')}
        <div className={styles.comm_table_container} ref={tableWrapperRef}>
          <CustomTableWithYScroll
            ...
            rowClassName={(record) => {
              ...
              //   return styles.table_content_disabled_bar;
              // } else
              if (record?.isPos === false) {
